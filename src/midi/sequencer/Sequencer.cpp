...
/**
 * Global singleton default sequencer environment.
 */
SeqEnvironment* SeqEnvironment::GlobalEnvironment = NULL;
Sequencer* SeqEnvironment::GlobalSequencer = NULL;

/**
 * New static method for the maintenance of a global 
 * Sequencer object.  Saves the application having to mess
 * with a SeqEnvironment and pass it around.
 */
INTERFACE Sequencer *SeqEnvironment::getSequencer(void)
{
	if (GlobalEnvironment == NULL) {
		GlobalEnvironment = SeqEnvironment::create();
		GlobalSequencer = GlobalEnvironment->newSequencer();
	}
	return GlobalSequencer;
}

/****************************************************************************
 * SeqEnvironment::create
 *
 * Arguments:
 *
 ...
 * Description: 
 * 
 * Creates and initializes a SeqEnvironment, and opens a SeqModule.
 ****************************************************************************/

INTERFACE SeqEnvironment *SeqEnvironment::create(void)
{
	SeqEnvironment *env;

	env = new SeqEnvironment;
	if (env != NULL) {
		// install SeqModule and related modules
		env->seqmodule = SeqModule::open(env);
		if (env->seqmodule == NULL) {
			// unable to open seq module
			delete env;
			env = NULL;
		}
	}
...
INTERFACE SeqEnvironment::~SeqEnvironment(void)
{
	// nothing to do, modules are destructed automatically
	// by BasicEnvironment
}
 ...
 *	env: environment
 *
 * Returns: MidiDev application object
 *
 * Description: 
 * 
 * Initializes the MidiDev global state within the given environment.
 * This will also initialize the MidiApplication which provides a factory
 * for the MidiEvent objects and other things.
 ****************************************************************************/

#define SEQ_MODULE_NAME "sequencer"

INTERFACE SeqModule *SeqModule::open(BasicEnvironment *env)
{
	EnvModule 		*existing;
	MidiModule		*midi;
	MmDevModule		*devs;
	...
	if (env != NULL) {
		...
		// be sure to define dependent modules FIRST so things
		// get destructed in the right order

		midi 	= MidiModule::open(env);
		devs	= MmDevModule::open(env);

		// see if we already have one
		existing = env->findModule(SEQ_MODULE_NAME);
		if (existing != NULL) {
			mod = (SeqModule *)existing;
		}
		else {
			mod	= new SeqModule;
			mod->setName(SEQ_MODULE_NAME);
			mod->midimodule 	= midi;
			mod->iomodule 		= devs;
			env->addModule(mod);
		}
 ...
 * Description: 
 * 
 * Called while the environment we're attached to is destructed.
 * Could free resources, but wait till destructor ? 
 * 
 ****************************************************************************/

INTERFACE void SeqModule::close(void)
{
}
 ...
 * Description: 
 * 
 * Destructs the sequencer global state.
 * This destroys any Sequencer objects that we created along the way.
 ****************************************************************************/

INTERFACE SeqModule::~SeqModule(void)
{
	Sequencer *s;

	// since sequencer destructors wack our list, be careful
	while (sequencers != NULL) {
		s = sequencers;
		removeSequencer(s);
		delete s;
	}
...
// In here to avoid having to include internal module headers
// when compiling applications that use seq.h
//

INTERFACE void SeqEnvironment::setTraceInterrupts(bool b) {
	seqmodule->getMidiDeviceModule()->setTraceInterrupts(b);
}

INTERFACE MidiSequence *Sequencer::newSequence(void) {

	return module->getMidiModule()->newSequence();
}

INTERFACE MidiEvent *Sequencer::newEvent(int status, int channel, 
										 int key, int velocity) {

	return module->getMidiModule()->newEvent(
		status, channel, key, velocity);
}


INTERFACE MMDeviceInfo* SeqEnvironment::getInputDevices()
{
	return seqmodule->getInputDevices();
}

INTERFACE MMDeviceInfo* SeqEnvironment::getOutputDevices()
{
	return seqmodule->getOutputDevices();
}

INTERFACE MMDeviceInfo* SeqModule::getInputDevices()
{
	return iomodule->getInputDevices();
}

INTERFACE MMDeviceInfo* SeqModule::getOutputDevices()
{
	return iomodule->getOutputDevices();
}
...
/****************************************************************************
 * SeqModule::newSequencer
 *
 * Arguments:
 *	none: 
 *
 * Returns: new sequencer object
 *
 * Description: 
 * 
 * Creates a new sequencer object.  
 * The sequencer is not yet connected to any devices, these must be specified
 * using the sequencer acces methods.
 ****************************************************************************/

INTERFACE Sequencer *SeqModule::newSequencer(void)
{
	return Sequencer::create(this);
}

PRIVATE void SeqModule::addSequencer(Sequencer *s) 
{
	s->setNext(sequencers);
	sequencers = s;
}

PRIVATE void SeqModule::removeSequencer(Sequencer *seq)
{
	Sequencer *s, *prev;

	prev = NULL;
	for (s = sequencers ; s != NULL && s != seq ; s = s->getNext())
	  prev = s;

	if (s != NULL) {
		if (prev == NULL)
		  sequencers = s->getNext();
		else
 ...
 * Description: 
 * 
 * Create a new sequencer object, these are always created within 
 * a SeqModule object which serves as the root of our global state.
 * 
 ****************************************************************************/
...
INTERFACE Sequencer *Sequencer::create(SeqModule *mod)
{
	Sequencer *s;
	...
	if (s != NULL) {
		...
		// always have an attached metronome object
		// adjust metronome for default beats-per-measure ? 
		s->metronome = new SeqMetronome;
		s->metronome->init();

		// We Always have a timer & default IO device objects, though they
		// may not always be active.

		Timer *t = mod->getMidiDeviceModule()->newTimer();
		t->setCallback(seq_timer_callback, s);
		s->timer  = t;

		// set up default input/output device
		MmDevModule *mm = mod->getMidiDeviceModule();
		MMDeviceInfo *inputs = mm->getInputDevices();

		if (inputs != NULL) {
			const char *devname = inputs->getName();
			mod->getEnv()->debug("Sequencer: Creating MidiIn for default port %s\n", 
								 devname);
			MidiIn *in = mm->newInput();
			in->setPort(devname);
			in->setCallback(seq_midi_in_callback, s);
			s->_inputs[0]  = in;
			s->_defaultInput = 0;
			s->_lastInput = 1;
		...
		MMDeviceInfo *outputs = mm->getOutputDevices();
		if (outputs != NULL) {
			const char *devname = outputs->getName();
			mod->getEnv()->debug("Sequencer: Creating MidiOut for default port %s\n", 
								 devname);
			MidiOut *out = mod->getMidiDeviceModule()->newOutput();
			out->setPort(devname);
			s->_outputs[0] = out;
			s->_defaultOutput = 0;
			s->_lastOutput = 1;
		...
		// we're disabling connection to the devices till we start,
		// do it now?

		s->mCsect = new CriticalSection;
	}
 ...
 * We're on a BasicList so the rest of the list is deleted automatically.
 ****************************************************************************/
...
INTERFACE Sequencer::~Sequencer(void)
{
	...
	// need to take this out of the module's list, be careful in ~SeqModule
	module->removeSequencer(this);
 ...
 * Arguments:
 *
 * Returns: non-zero if the sequencer could not be activated
 *
 * Description: 
 * 
 * Activates a sequencer by in turn activating its timer, input
 * and output devices.  This prepares the sequencer for use, if you
 * only have one, it is ordinarilly always active, but if there
 * are other threads in the same application that maintain midi
 * device state or other sequencer objects, then activation must
 * be passed among them.
 * 
 * Hmm, we probably need to look at the peer sequencers maintained
 * in our SeqModule and do a mutex activation.
 * 
 ****************************************************************************/

INTERFACE int Sequencer::activate(void)
{
	int error = 0;

	// if we're running, we presumably have to be active
	if (!running) {

		debug("Sequencer::activate\n");

		// should only activate on demand!
		for (int i = 0 ; i < SEQ_MAX_PORT && !error ; i++) {
			
			if (_inputs[i] != NULL)
			  error = _inputs[i]->connect();
			
			if (!error && _outputs[i] != NULL)
			  error = _outputs[i]->connect();
		}

		debug("Sequencer::activate finished\n");

		// timer is auto-activated when we start the clock, is
		// that enough?
	}
 ...
 * Description: 
 * 
 * Deactivates a sequencer by in turn deactivating its timer, input
 * and output devices.  This ordinarilly needs to be done only
 * if we have several sequencer objects and control needs to pass
 * between them.
 * 
 ****************************************************************************/

INTERFACE void Sequencer::deactivate(void)
{
	// should we pause instead?
	stop();
	...
	for (int i = 0 ; i < SEQ_MAX_PORT ; i++) {
		...
		if (_outputs[i] != NULL) 
		  _outputs[i]->disconnect();
	}

	// don't have disconnect for the timer, should we?
}
...
{
	float tempo = timer->getTempo();
	// module->getEnv()->debug("Sequencer: Getting tempo %f\n", tempo);
	return tempo;
}
...
{
	return timer->getBeatsPerMeasure();
}

INTERFACE void Sequencer::setBeatsPerMeasure(int b) 
{
	// ordinarilly the clock should be stopped, but what the heck
	timer->setBeatsPerMeasure(b);
	metronome->setBeat(b);
}
...
INTERFACE void Sequencer::setClock(int c)
{
	...
	// adjust metrnome beat counter
	metronome->setClock(c);
...
{
	stop();
	timer->setSongPosition(psn);
	// adjust metrome based on clock after song position was set
	metronome->setClock(timer->getClock());
}
...
PRIVATE void Sequencer::startInternal(int do_callback)
{
	if (!running) {
		...
		if (recording)
		  recording->start(timer->getClock());
		...
		if (do_callback && callback_command != NULL)
		  (*callback_command)(this, 1, 0);
		
		// do this now?
		if (mEventMask & SeqEventStart)
		  addEvent(SeqEventStart, timer->getClock(), 0, 0);
...
INTERFACE void Sequencer::start(void)
{
	...
	else if (recording != NULL && loop_start_enable)
	  setClock(loop_start);
...
PRIVATE void Sequencer::stopInternal(int do_callback)
{
	...
	if (running) {
		...
		if (sweeping) {
			...
			if (do_callback && callback_command != NULL)
			  (*(callback_command))(this, 0, neu);      

			if (mEventMask & SeqEventStop)
			  addEvent(SeqEventStop, timer->getClock(), 0, 0);
		}
	}
}
...
INTERFACE int Sequencer::openInputPort(const char *portName)
{
	int port = -1;

	// look for an existing one
	if (_inputs != NULL) {
		for (int i = 0 ; i < _lastInput ; i++) {
			MidiIn *in = _inputs[i];
			if (in != NULL && !strcmp(portName, in->getPortName())) {
				port = i;
				break;
			}
		}
	...
	if (port == -1) {
		port = _lastInput;
		_lastInput++;
		debug("Sequencer: Creating MidiIn for port %s\n", portName);
		MidiIn *in = module->getMidiDeviceModule()->newInput();
		in->setCallback(seq_midi_in_callback, this);
		// !! todo, check validity and ignore
		in->setPort(portName); 
		_inputs[port] = in;
	}
...
INTERFACE int Sequencer::openInputPort(int id)
{
	int port = -1;
	...
	if (port == -1) {
		port = _lastInput;
		_lastInput++;
		debug("Sequencer: Creating MidiIn for port %d\n", id);
		MidiIn *in = module->getMidiDeviceModule()->newInput();
		in->setCallback(seq_midi_in_callback, this);
		in->setPort(id);
		_inputs[port] = in;
...
INTERFACE void Sequencer::removeInputPort(int id)
{
	int index = -1;
	...
	if (index >= 0) {
		for (int i = index ; i < _lastInput - 1 ; i++) 
		  _inputs[i] = _inputs[i + 1];
		_inputs[_lastInput - 1] = NULL;
		_lastInput--;
	}
}

INTERFACE void Sequencer::setDefaultInputPort(int port) 
{
	stop();
	if (port >= 0 && port <= _lastInput)
	  _defaultInput = port;
}

INTERFACE int Sequencer::openOutputPort(const char *portName)
{
	int port = -1;

	// look for an existing one
	if (_outputs != NULL) {
		for (int i = 0 ; i < _lastOutput ; i++) {
			MidiOut *out = _outputs[i];
			if (out != NULL && !strcmp(portName, out->getPortName())) {
				port = i;
				break;
			}
		}
	...
	if (port == -1) {
		port = _lastOutput;
		_lastOutput++;
		debug("Sequencer: Creating MidiOut for port %s\n", portName);
		MidiOut *out = module->getMidiDeviceModule()->newOutput();
		out->setPort(portName);
		_outputs[port] = out;
	}
...
INTERFACE int Sequencer::openOutputPort(int id)
{
	int port = -1;
	...
	if (port == -1) {
		port = _lastOutput;
		_lastOutput++;
		debug("Sequencer: Creating MidiOut for port %d\n", id);
		MidiOut *out = module->getMidiDeviceModule()->newOutput();
		out->setPort(id);
		_outputs[port] = out;
	}
...
INTERFACE void Sequencer::setDefaultOutputPort(int port) 
{
	stop();
	if (port >= 0 && port <= _lastOutput)
	  _defaultOutput = port;
}

INTERFACE void Sequencer::removeOutputPort(int id)
{
	int index = -1;
	...
	if (index >= 0) {
		for (int i = index ; i < _lastOutput - 1 ; i++) 
		  _inputs[i] = _inputs[i + 1];
		_inputs[_lastOutput - 1] = NULL;
		_lastOutput--;
	}
}
...
INTERFACE int Sequencer::areNotesHanging(void)
{
	...
	if (!hanging && recording != NULL && recording->getOn() != NULL)
	  hanging = 1;
...
INTERFACE void Sequencer::setMidiSync(int port, int enable) 
{
	...
	timer->setMidiDevice(output);
	timer->setMidiSync(enable);
}
