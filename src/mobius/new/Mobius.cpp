...
PUBLIC Mobius::Mobius(MobiusContext* context)
{
	...
	// need this to manage the action list, other things
    ...
    mFunctions = NULL;
	mScriptPackage = NULL;
	mScripts = new ScriptRuntime(this);
    mDispatcher = new ActionDispatcher(this, mCsect, mScripts);
...
PUBLIC void Mobius::start()
{
	if (mRecorder == NULL) {
        ...
        mTransport = new MidiTransport(mMidi, getSampleRate());
		mSynchronizer = new Synchronizer(this, mMidi, mTransport);
        ...
        // Never used this and it didn't fit well in the new ScriptPackage world.
 ...
 */
PUBLIC bool Mobius::isFocused(Track* t) 
{
    ...
    for (ResolvedTarget* t = mResolvedTargets ; t != NULL ; t = t->getNext()) {
        ...
        if (target == TargetFunction) {
            ...
            if (f != NULL && f->isScript()) {
                ...
                f->object = mScriptPackage->getScript(script);
            }
        }
...
PUBLIC Action* Mobius::getScriptButtonActions()
{
    ...
    for (Script* script = mScriptPackage->getScripts() ; script != NULL ;
...
/**
 * Initialize script parameters after installing a ScriptPackage.
 */
...
PRIVATE void Mobius::initFunctions()
{
	...
	List* scripts = NULL;
	if (mScriptPackage != NULL) {
		scripts = mScriptPackage->getScriptFunctions();
		if (scripts != NULL)
...
{
    mScripts->cancelScripts(action, t);
}
