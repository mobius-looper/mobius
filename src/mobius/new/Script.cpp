 ...
 * 
 * Link
 *   Call references between scripts in the ScriptPackage are resolved.
...
/**
 * Called when the entire ScriptPackage has been loaded and the
...
/**
 * Resolve a call to another script in the package.
 */
...
PUBLIC Script::Script(ScriptPackage* package, const char* filename)
{
	init();
	mPackage = package;
...
PUBLIC void Script::setPackage(ScriptPackage* pkg)
{
    mPackage = pkg;
}

PUBLIC ScriptPackage* Script::getPackage()
{
    return mPackage;
}
...
/**
 * Resolve references between scripts after the entire package
...
PUBLIC ScriptPackage* ScriptPackage::getNext()
{
    return mNext;
}

PUBLIC void ScriptPackage::setNext(ScriptPackage* package)
{
    mNext = package;
}

PUBLIC ScriptConfig* ScriptPackage::getSource()
{
    return mSource;
}

PUBLIC void ScriptPackage::setSource(ScriptConfig* config)
{
    delete mSource;
    mSource = config;
}

PUBLIC Script* ScriptPackage::getScripts()
{
	return mScripts;
}

PUBLIC void ScriptPackage::setScripts(Script* scripts)
{
    delete mScripts;
    mScripts = scripts;
}
 ...
 * table.
 */
PUBLIC List* ScriptPackage::getScriptFunctions()
{
	List* functions = NULL;
 ...
 */
PUBLIC bool ScriptPackage::isDifference(ScriptConfig* config)
{
 ...
 */
PUBLIC Script* ScriptPackage::getScript(Script* src)
{
    Script* found = NULL;
 ...
 * 
 * The returned ScriptPackage is completely self contained and only has
 ...
 * MobiusContext.
 */
PUBLIC ScriptPackage* ScriptCompiler::compile(Mobius* m, ScriptConfig* config)
{
    // should not try to use this more than once
    if (mPackage != NULL)
      Trace(1, "ScriptCompiler: dangling package!\n");

    mMobius = m;
    mPackage = new ScriptPackage();
    mScripts = NULL;
    mLast = NULL;

    // give it a copy of the config for later diff detection
    mPackage->setSource(config->clone());
 ...
 * with substitution in the ScriptPackage and elsewhere.
	...
	if (script->isAutoLoad()) {
        ...
        if (filename != NULL) {
            ...
            if (fp == NULL) {
                ...
                // Get the package for linking script references
                mPackage = script->getPackage();
...
PRIVATE void ScriptCompiler::parse(const char* filename)
{
    if (!IsFile(filename)) {
        ...
        if (fp == NULL) {
            ...
            Script* script = new Script(mPackage, filename);
 ...
 * first case we only look at the local mScripts list since mPackage
 * may have things that are no longer configured.  In the second
 * case we must use what is in mPackage.
 */
PUBLIC Script* ScriptCompiler::resolveScript(const char* name)
{
    ...
    if (mScripts != NULL) {
        ...
        // fall back to ScriptPackage
        found = resolveScript(mPackage->getScripts(), name);
    }
...
PRIVATE Script* ScriptCompiler::resolveScript(Script* scripts, const char* name)
{
    ...
    if (found != NULL) {
		Trace(2, "ScriptPackage: Reference %s resolved to script %s\n",
