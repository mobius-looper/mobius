...
PUBLIC TrackNumber::TrackNumber(MobiusInterface* m, int trackIndex) :
    FocusButton(m, trackIndex)
{
...
PUBLIC TrackStrip2::TrackStrip2(MobiusInterface* m, int track) :
    TrackStrip(m, track)
{
...
PUBLIC void TrackStrip::updateConfiguration(StringList* controls)
{
    initComponents();
    ...
    if (isEnabled() && controls != NULL) {
        for (int i = 0 ; i < controls->size() ; i++) {
            ...
            if (el == FocusLockElement) {
                ...
                mLevel->setRequiredSize(new Dimension(75, 10));
                add(mLevel);
            }
...
PUBLIC void TrackStrip::update(MobiusState* mstate)
{
	...
	if (mGroup != NULL) mGroup->update(tstate->group);

    if (mInput != NULL) mInput->update();

    if (mOutput != NULL) mOutput->update();

    if (mFeedback != NULL) mFeedback->update();

    if (mAltFeedback != NULL) mAltFeedback->update();
    
    if (mPan != NULL) mPan->update();

    if (mSpeedOctave != NULL) mSpeedOctave->update();

    if (mSpeedStep != NULL) mSpeedStep->update();

    if (mSpeedBend != NULL) mSpeedBend->update();

    if (mPitchOctave != NULL) mPitchOctave->update();

    if (mPitchStep != NULL) mPitchStep->update();

    if (mPitchBend != NULL) mPitchBend->update();

    if (mTimeStretch != NULL) mTimeStretch->update();
